# Changelog

## 0.3.1
- Refined the public product surface and repository documentation.
- Reconciled release identity across the maintained public release files.
- Re-reviewed a time-sensitive reported development without changing its evidence state.
- Strengthened repository-wide public-boundary validation while preserving public evidence and provenance.

## 0.2.8
- Public surface and maintenance cleanup.

## 0.2.7
- UX and maintenance improvements.

## 0.2.6
- Validation and accessibility improvements.

## 0.2.5
- Public intelligence experience update.

## 0.2.4
- Learning and usability improvements.

## 0.2.3
- Accessibility and layout improvements.

## 0.2.2
- Repository validation improvements.

## 0.2.1
- Documentation and maintenance updates.

## 0.2.0
- Usability and validation improvements.

## 0.1.2
- Accessibility and licensing updates.

## 0.1.1
- Source attribution correction.

## 0.1.0
- Initial public release.
