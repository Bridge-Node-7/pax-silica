...
class P(HTMLParser):
    def __init__(self):super().__init__();self.ids=[];self.h1=0;self.main=0;self.refs=[]
...
class HtmlTests(unittest.TestCase):
    def test_html_contract(self):
        with tempfile.TemporaryDirectory() as a:
            ...
            p=P();p.feed(text)
            self.assertEqual(p.h1,1);self.assertEqual(p.main,1);self.assertEqual(len(p.ids),len(set(p.ids)))
            self.assertIn('<link rel="canonical" href="https://bridgenode7.com/pax-silica/">',text)
